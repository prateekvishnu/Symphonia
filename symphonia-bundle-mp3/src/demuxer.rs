...
/// Reads a MPEG frame and checks if the next frame begins after the packet, and that it has a
/// similar header.
fn read_mpeg_frame_strict(reader: &mut MediaSourceStream) -> Result<(FrameHeader, Vec<u8>)> {
    loop {
        ...
        // Read the next four bytes. If this read fails then the file may have ended and this check
        // cannot be performed.
